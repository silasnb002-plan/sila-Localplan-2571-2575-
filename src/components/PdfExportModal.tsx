...
import { YEARS, ORG_NAME, ORG_PROVINCE, STANDARD_STRATEGIC_ISSUES, sortStrategicIssues, STANDARD_DEPARTMENTS, sortDepartments } from '../data/initialData';
...
export const PdfExportModal: React.FC<PdfExportModalProps> = ({
  ...
  const groupedProjects = useMemo(() => {
    ...
    return order.map((issue) => ({ issue, items: map[issue] }));
  }, [filteredProjects]);
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-start justify-between border-b pb-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-600">
                          แผนพัฒนาท้องถิ่น (พ.ศ. 2571–2575) — {ORG_NAME}
                          {reportType === 'ผ02-additional' && ` (ฉบับเพิ่มเติม ครั้งที่ ${revisionNumber})`}
                          {reportType === 'change-diff' && ` (ฉบับเปลี่ยนแปลง ครั้งที่ ${revisionNumber})`}
                          {reportType === 'edit-diff' && ` (ฉบับแก้ไข ครั้งที่ ${revisionNumber})`}
                        </p>
                      </div>
                      <div className="text-right text-[11px] font-bold text-slate-700">
                        แบบ ผ.02
                    ...
                    {groupedProjects.map((grp) => {
                      ...
                      return (
                        <div key={grp.issue} className="space-y-2">
                          <div className="bg-slate-100 p-2 rounded border border-slate-300 font-bold text-slate-900 flex items-center justify-between">
                            <span>ประเด็นการพัฒนา: {grp.issue}</span>
                            <span className="text-xs text-slate-600 font-normal">
                              จำนวน {grp.items.length} โครงการ
                            </span>
                          </div>

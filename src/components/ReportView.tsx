...
import { YEARS, ORG_NAME, ORG_PROVINCE, TYPE_LIST, STANDARD_STRATEGIC_ISSUES, sortStrategicIssues, STANDARD_DEPARTMENTS, sortDepartments } from '../data/initialData';
...
export const ReportView: React.FC<ReportViewProps> = ({
  ...
  const availableDepartments = useMemo(() => {
    ...
    projects.forEach((p) => {
      if (p['หน่วยงานรับผิดชอบหลัก']) set.add(p['หน่วยงานรับผิดชอบหลัก'].trim());
    });
    return sortDepartments(Array.from(set));
  }, [projects]);
      ...
      <div className="bg-white rounded-lg p-3 sm:p-5 border border-slate-200 shadow-2xs report-sheet overflow-auto max-h-[calc(100vh-175px)] custom-scrollbar flex-1 print:max-h-none print:overflow-visible print:p-0 print:border-none print:shadow-none">
                      ...
                      <table className="w-full text-xs text-left border-collapse border border-slate-900">
                        <thead>
                          <tr className="bg-slate-50 text-slate-900 border-b border-slate-900 text-center font-bold">
                            <th
                              rowSpan={2}
                              className="py-3 px-2 border border-slate-900 font-bold w-10 text-center align-middle bg-slate-100/90"
                            >
                              ที่
                            </th>
                            <th
                              rowSpan={2}
                              className="py-3 px-3 border border-slate-900 font-bold min-w-[200px] text-center align-middle bg-slate-100/80"
                            >
                              โครงการ
                            </th>
                            <th
                              rowSpan={2}
                              className="py-3 px-3 border border-slate-900 font-bold min-w-[170px] text-center align-middle bg-slate-100/80"
                            >
                              วัตถุประสงค์
                            </th>
                            <th
                            ...
                            <th
                              rowSpan={2}
                              className="py-3 px-3 border border-slate-900 font-bold min-w-[150px] text-center align-middle bg-slate-100/80"
                            >
                              ผลที่คาดว่า<br />จะได้รับ
                            </th>
                            <th
                              rowSpan={2}
                              className="py-3 px-3 border border-slate-900 font-bold min-w-[120px] text-center align-middle bg-slate-100/80"
                            >
                          ...
                          <tr className="bg-slate-100 text-slate-900 border-b border-slate-900 text-[11px] text-center font-bold">
                            {YEARS.map((y) => (
                              <th
                                key={y}
                                className="py-1.5 px-1.5 border border-slate-900 min-w-[80px] text-center align-middle"
                              >
                ...
                return (
                  <div key={group.issue} className="mt-8 space-y-2.5">
                    ...
                    <div className="overflow-x-auto rounded-lg border border-slate-900 shadow-2xs">
                      <table className="w-full text-xs text-left border-collapse border border-slate-900 bg-white">
                        <thead>
                          <tr className="bg-emerald-50/70 text-slate-900 border-b border-slate-900 text-center font-bold">
                            <th rowSpan={2} className="py-3 px-2 border border-slate-900 font-bold w-10 text-center align-middle bg-slate-100/90">
                              ที่
                            </th>
                            <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[200px] text-center align-middle bg-slate-100/80">
                              โครงการ
                            </th>
                            <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[160px] text-center align-middle bg-slate-100/80">
                              วัตถุประสงค์
                            </th>
                            <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[160px] text-center align-middle bg-slate-100/80">
                              เป้าหมาย<br />(ผลผลิตของโครงการ)
                            </th>
                            <th colSpan={YEARS.length} className="py-2 px-2 text-center border border-slate-900 font-bold bg-emerald-50/80">
                              งบประมาณ (บาท)
                            </th>
                            <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[140px] text-center align-middle bg-slate-100/80">
                              ผลที่คาดว่า<br />จะได้รับ
                            </th>
                            <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[120px] text-center align-middle bg-slate-100/80">
                              หน่วยงาน<br />รับผิดชอบ
                            </th>
                            <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[160px] text-center align-middle bg-emerald-50/60">
                              เหตุผลความจำเป็น
                            </th>
                          </tr>
                          <tr className="bg-slate-100/90 text-slate-900 border-b border-slate-900 text-[11px] text-center font-bold">
                            {YEARS.map((y) => (
                              <th key={y} className="py-1.5 px-1.5 border border-slate-900 min-w-[80px] text-center align-middle">
                                {formatDigits(y)}
                              </th>
                            ))}
                          </tr>
                        ...
                        <tbody>
                          {group.items.map((p, pIdx) => {
                            const reasonText = (p['เหตุผลและความจำเป็น'] || p['เหตุผลความจำเป็น'] || '').trim();
                            const hasReason = reasonText !== '' && reasonText !== '-';

                            return (
                              <tr key={p.ID} className="hover:bg-slate-50/80 border-b border-slate-900 transition-colors">
                                <td className="py-2.5 px-2 text-center border border-slate-900 font-bold align-middle font-mono text-xs text-slate-800 bg-slate-50/30">
                                  {formatDigits(pIdx + 1)}
                                </td>
                                <td className="py-2.5 px-2.5 border border-slate-900 font-semibold text-slate-900 align-top leading-snug">
                                  <div className="font-bold text-slate-900">{p['ชื่อโครงการ'] || p.project_name}</div>
                                </td>
                                <td className="py-2.5 px-2.5 border border-slate-900 text-slate-800 align-top leading-relaxed whitespace-pre-line text-[11px]">
                                  {p['วัตถุประสงค์'] || p.objective || '-'}
                                </td>
                                <td className="py-2.5 px-2.5 border border-slate-900 text-slate-800 align-top leading-relaxed whitespace-pre-line text-[11px]">
                                  {p['เป้าหมาย (ผลผลิต)'] || p.target || '-'}
                                </td>

                                {/* 5-Year Budget Columns */}
                                {YEARS.map((y) => {
                                  const val = Number(p[`งบประมาณ ${y}` as keyof Project] ?? (p as any)[`budget_${y}`]) || 0;
                                  return (
                                    <td
                                      key={y}
                                      className="py-2.5 px-2 text-right border border-slate-900 font-mono align-top whitespace-nowrap text-[11px]"
                                    >
                                      {formatMoney(val)}
                                    </td>
                                  );
                                ...
                                <td className="py-2.5 px-2.5 border border-slate-900 text-slate-800 align-top leading-relaxed text-[11px]">
                                  {p['ผลที่คาดว่าจะได้รับ'] || p.expected_outcome || '-'}
                                </td>
                                <td className="py-2.5 px-2.5 border border-slate-900 text-slate-900 font-medium align-top text-[11px]">
                                  {p['หน่วยงานรับผิดชอบหลัก'] || p.department || '-'}
                                </td>
                                <td className="border border-slate-900 align-middle" style={{ padding: '12px' }}>
                                  {!hasReason ? (
                                    <div className="flex items-center justify-center">
                                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium bg-slate-100 text-slate-500 border border-slate-200 shadow-2xs">
                                    ...
                                    <div
                                      className="text-[11px] whitespace-pre-line rounded-md bg-emerald-50/40 p-2.5 border border-emerald-200/80 shadow-2xs"
                                      style={{
                                        lineHeight: '1.6',
                                        color: '#334155'
                                      }}
        ...
        {activeTab === 'change-diff' && (
          <div className="space-y-6">
            ...
            {/* Official Center Title with Balanced Margins */}
            <div className="text-center my-6 space-y-1.5 max-w-3xl mx-auto">
            ...
            {changeGroups.length > 0 ? (
              changeGroups.map((group, gIdx) => (
                <div key={group.issue} className="mt-8 space-y-3">
                  {/* Issue Section Header */}
                  <div className="text-xs sm:text-sm font-bold text-slate-900 flex items-center gap-2">
                    <span className="inline-block w-2 h-2 rounded-full bg-emerald-600"></span>
                    <span>{formatDigits(gIdx + 1)}. ประเด็นการพัฒนาท้องถิ่น: {group.issue}</span>
                  </div>

                  <div className="overflow-x-auto rounded-lg border border-slate-900 shadow-2xs">
                    <table className="w-full text-xs text-left border-collapse border border-slate-900 bg-white">
                      <thead>
                        <tr className="bg-emerald-50/70 text-slate-900 border-b border-slate-900 text-center font-bold text-xs">
                          <th className="py-3.5 px-2 border border-slate-900 w-12 text-center align-middle bg-slate-100/90 font-bold">
                            ที่
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[280px] text-center align-middle bg-slate-100/80 font-bold">
                            โครงการเดิม<br />(ตามแผนพัฒนาท้องถิ่น)
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[280px] text-center align-middle bg-purple-50/90 font-bold text-purple-950">
                            โครงการที่ขอเปลี่ยนแปลง<br />(ฉบับเปลี่ยนแปลง)
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[220px] text-center align-middle bg-emerald-50/60 font-bold text-slate-900">
                            เหตุผลและความจำเป็น<br />ในการเปลี่ยนแปลง
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[150px] text-center align-middle bg-slate-100/90 font-bold text-slate-900">
                      ...
                      <tbody>
                        {group.items.map((p, idx) => {
                          ...
                          YEARS.forEach((y) => {
                            ...
                            sumAfter += Number(p[`งบประมาณ ${y}` as keyof Project] ?? (p as any)[`budget_${y}`]) || 0;
                          });
                          ...
                          return (
                            <tr key={p.ID} className="hover:bg-slate-50/80 border-b border-slate-900 transition-colors">
                              {/* 1. ที่ (Row Number) */}
                              <td className="py-3 px-2 text-center border border-slate-900 font-bold align-middle font-mono text-xs text-slate-800 bg-slate-50/30">
                                {formatDigits(idx + 1)}
                              </td>

                              {/* 2. โครงการเดิม */}
                              <td className="py-3 px-3 border border-slate-900 align-top bg-slate-50/40 space-y-1.5">
                                <div className="font-bold text-slate-900 leading-snug">
                                  {p['ชื่อโครงการ (เดิม)'] || p['ชื่อโครงการ'] || p.project_name}
                                </div>
                                <div className="text-[11px] text-slate-700 leading-relaxed">
                                  <span className="font-semibold text-slate-900">วัตถุประสงค์:</span>{' '}
                                  {p['วัตถุประสงค์ (เดิม)'] || p['วัตถุประสงค์'] || p.objective || '-'}
                                </div>
                                <div className="text-[11px] text-slate-700 leading-relaxed">
                                  <span className="font-semibold text-slate-900">เป้าหมาย (ผลผลิต):</span>{' '}
                                  {p['เป้าหมาย (เดิม)'] || p['เป้าหมาย (ผลผลิต)'] || p.target || '-'}
                                </div>
                                <div className="text-[11px] font-mono text-slate-800 bg-white p-2 rounded-md border border-slate-200 shadow-2xs">
                                  <span className="font-bold font-sans text-slate-700">งบประมาณรวม {formatDigits(5)} ปี: </span>
                                  <span className="font-bold text-slate-900">{formatMoney(sumBefore)}</span> บาท
                                </div>
                                <div className="text-[10px] text-slate-500 font-medium">
                                  หน่วยงานรับผิดชอบ: {p['หน่วยงานรับผิดชอบหลัก (เดิม)'] || p['หน่วยงานรับผิดชอบหลัก'] || p.department || '-'}
                                </div>
                              </td>

                              {/* 3. โครงการที่ขอเปลี่ยนแปลง */}
                              <td className="py-3 px-3 border border-slate-900 align-top bg-purple-50/30 space-y-1.5">
                                <div className="font-bold text-purple-950 leading-snug">
                                  {p['ชื่อโครงการ'] || p.project_name}
                                </div>
                                ...
                                <div className="text-[11px] font-mono text-purple-950 bg-white p-2 rounded-md border border-purple-200 shadow-2xs">
                                  <span className="font-bold font-sans text-purple-900">งบประมาณรวม {formatDigits(5)} ปี: </span>
                                  <span className="font-bold text-purple-950">{formatMoney(sumAfter)}</span> บาท
                                </div>
                                <div className="text-[10px] text-purple-700 font-medium">
                                  หน่วยงานรับผิดชอบ: {p['หน่วยงานรับผิดชอบหลัก'] || p.department || '-'}
                                </div>
                              </td>

                              {/* 4. เหตุผลและความจำเป็นในการเปลี่ยนแปลง */}
                              <td className="border border-slate-900 align-middle" style={{ padding: '12px' }}>
                                {!hasReason ? (
                                  <div className="flex items-center justify-center">
                                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[11px] font-medium bg-slate-100 text-slate-500 border border-slate-200 shadow-2xs">
                                      <span className="w-1.5 h-1.5 rounded-full bg-slate-400"></span>
                                      ไม่ได้ระบุเหตุผล
                                    </span>
                                  </div>
                                  ...
                                  <div
                                    className="text-[11px] whitespace-pre-line rounded-md bg-slate-50/70 p-2.5 border border-slate-200/80 shadow-2xs"
                                    style={{
                                      lineHeight: '1.6',
                                      color: '#334155'
                                    }}
                              ...
                              {/* 5. ส่วนต่างงบประมาณ (เพิ่ม / ลด) */}
                              <td className="py-3 px-3 text-center border border-slate-900 align-middle">
                                {diff === 0 ? (
                                  <span className="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-600 border border-slate-200 shadow-2xs whitespace-nowrap">
                                    ไม่เปลี่ยนแปลง
                                  </span>
                                ) : diff > 0 ? (
                                  <span className="inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold font-mono bg-emerald-50 text-emerald-800 border border-emerald-300 shadow-2xs whitespace-nowrap">
                                    <span className="font-sans text-[11px] font-semibold text-emerald-700">เพิ่ม</span>
                                    <span>+{formatMoney(diff)}</span>
                                    <span className="font-sans text-[10px] font-normal text-emerald-700">บาท</span>
                                  </span>
                                ) : (
                                  <span className="inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold font-mono bg-rose-50 text-rose-800 border border-rose-300 shadow-2xs whitespace-nowrap">
                                    <span className="font-sans text-[11px] font-semibold text-rose-700">ลด</span>
                                    <span>-{formatMoney(Math.abs(diff))}</span>
                                    <span className="font-sans text-[10px] font-normal text-rose-700">บาท</span>
                                  </span>
                                )}
                              </td>
                            </tr>
              ...
              editGroups.map((group, gIdx) => (
                <div key={group.issue} className="mt-8 space-y-3">
                  ...
                  <div className="overflow-x-auto rounded-lg border border-slate-900 shadow-2xs">
                    <table className="w-full text-xs text-left border-collapse border border-slate-900 bg-white">
                      <thead>
                        <tr className="bg-amber-50/70 text-slate-900 border-b border-slate-900 text-center font-bold text-xs">
                          <th className="py-3.5 px-2 border border-slate-900 w-12 text-center align-middle bg-slate-100/90 font-bold">
                            ที่
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[260px] text-center align-middle bg-slate-100/80 font-bold">
                            ข้อความ/รายการเดิม<br />(ก่อนแก้ไข)
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[260px] text-center align-middle bg-amber-50/90 font-bold text-amber-950">
                            ข้อความ/รายการใหม่<br />(หลังแก้ไข)
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[220px] text-center align-middle bg-slate-50/90 font-bold text-slate-900">
                            เหตุผลและความจำเป็นในการแก้ไข
                          </th>
                          <th className="py-3.5 px-3 border border-slate-900 min-w-[130px] text-center align-middle bg-slate-100/90 font-bold text-slate-900">
                            อำนาจอนุมัติ
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {group.items.map((p, idx) => {
                          const reasonText = (p['เหตุผลและความจำเป็น'] || p['เหตุผลความจำเป็น'] || 'แก้ไขข้อความให้ถูกต้องตรงตามข้อเท็จจริง').trim();
                          const hasReason = reasonText !== '' && reasonText !== '-';
                          ...
                          return (
                            <tr key={p.ID} className="hover:bg-slate-50/80 border-b border-slate-900 transition-colors">
                              ...
                              <td className="py-3 px-3 border border-slate-900 align-top bg-slate-50/40 space-y-1.5">
                                ...
                                {(p['วัตถุประสงค์ (เดิม)'] || p.objective) && (
                                  <div className="text-[11px] text-slate-700 leading-relaxed">
                                    <span className="font-semibold text-slate-900">วัตถุประสงค์:</span> {p['วัตถุประสงค์ (เดิม)'] || p.objective}
                                  </div>
                                )}
                                {(p['เป้าหมาย (เดิม)'] || p.target) && (
                                  <div className="text-[11px] text-slate-700 leading-relaxed">
                                    <span className="font-semibold text-slate-900">เป้าหมาย:</span> {p['เป้าหมาย (เดิม)'] || p.target}
                                  </div>
                                )}
                                {(p['หน่วยงานรับผิดชอบหลัก (เดิม)'] || p.department) && (
                                  <div className="text-[10px] text-slate-500 font-medium">
                                    <span className="font-semibold text-slate-700">หน่วยงาน:</span> {p['หน่วยงานรับผิดชอบหลัก (เดิม)'] || p.department}
                                  </div>
                                )}
                              </td>

                              {/* ข้อความใหม่ */}
                              <td className="py-3 px-3 border border-slate-900 align-top bg-amber-50/30 space-y-1.5">
                                <div className="font-bold text-amber-950 leading-snug">
                                  {p['ชื่อโครงการ'] || p.project_name}
                                </div>
                                <div className="text-[11px] text-slate-800 leading-relaxed">
                                  <span className="font-semibold text-amber-900">วัตถุประสงค์:</span> {p['วัตถุประสงค์'] || p.objective || '-'}
                                </div>
                                <div className="text-[11px] text-slate-800 leading-relaxed">
                                  <span className="font-semibold text-amber-900">เป้าหมาย:</span> {p['เป้าหมาย (ผลผลิต)'] || p.target || '-'}
                                </div>
                                <div className="text-[10px] text-amber-800 font-medium">
                                  <span className="font-semibold text-amber-950">หน่วยงาน:</span> {p['หน่วยงานรับผิดชอบหลัก'] || p.department || '-'}
                                </div>
                              </td>
                              ...
                              <td className="border border-slate-900 align-middle" style={{ padding: '12px' }}>
                                {!hasReason ? (
                                  ...
                                  <div
                                    className="text-[11px] whitespace-pre-line rounded-md bg-amber-50/50 p-2.5 border border-amber-200/80 shadow-2xs"
                                    style={{
                                      lineHeight: '1.6',
                                      color: '#334155'
                                    }}
                              ...
                              {/* อำนาจอนุมัติ */}
                              <td className="py-3 px-3 text-center border border-slate-900 align-middle">
                                <span className="inline-flex items-center justify-center px-3 py-1.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-700 border border-slate-200 shadow-2xs">
                                  ผู้บริหารท้องถิ่น
                                </span>
                              </td>
                            </tr>
                    ...
                    <table className="w-full text-xs text-left border-collapse border border-slate-900">
                      <thead>
                        <tr className="bg-slate-50 text-slate-900 border-b border-slate-900 text-center font-bold">
                          ...
                          <th colSpan={YEARS.length} className="py-2 px-2 text-center border border-slate-900 font-bold bg-slate-100">
                            งบประมาณ (บาท)
                          </th>
                          <th rowSpan={2} className="py-3 px-3 border border-slate-900 font-bold min-w-[140px] text-center align-middle bg-slate-100/80">
                            ผลที่คาดว่า<br />จะได้รับ
